...
type RestOf<T extends IRequest | NewRequest> = Omit<T, 'createddate' | 'updatedate'> & {
...
export class RequestService {
  ...
  protected convertDateFromClient<T extends IRequest | NewRequest | PartialUpdateRequest>(request: T): RestOf<T> {
    return {
      ...request,
      createddate: request.createddate?.toJSON() ?? null,
      updatedate: request.updatedate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restRequest: RestRequest): IRequest {
    return {
      ...restRequest,
      createddate: restRequest.createddate ? dayjs(restRequest.createddate) : undefined,
      updatedate: restRequest.updatedate ? dayjs(restRequest.updatedate) : undefined,
    };
  }
